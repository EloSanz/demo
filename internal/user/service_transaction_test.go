...
func (m *mockRepo) FindByID(ctx context.Context, id int64) (*user.User, error) { args := m.Called(ctx, id); return args.Get(0).(*user.User), args.Error(1) }
func (m *mockRepo) FindByEmail(ctx context.Context, email string) (*user.User, error) { args := m.Called(ctx, email); if args.Get(0) == nil { return nil, args.Error(1) }; return args.Get(0).(*user.User), args.Error(1) }
func (m *mockRepo) FindAll(ctx context.Context, page, size int) ([]user.User, int64, error) { args := m.Called(ctx, page, size); return args.Get(0).([]user.User), int64(args.Int(1)), args.Error(2) }
func (m *mockRepo) Save(ctx context.Context, u user.User) (user.User, error) { args := m.Called(ctx, u); return args.Get(0).(user.User), args.Error(1) }
...
func (m *mockExternal) FetchAll(ctx context.Context) ([]user.User, error) { args := m.Called(ctx); return args.Get(0).([]user.User), args.Error(1) }

type mockNotif struct{ mock.Mock }
func (m *mockNotif) Publish(ctx context.Context, n notification.Notification) error { args := m.Called(ctx, n); return args.Error(0) }
...
func TestUserService_SyncFromExternal_RollbackOnNotificationFailure(t *testing.T) {
	...
	// Configuramos el éxito del primer paso (DB)
	...
	// CONFIGURAMOS EL FALLO DEL SEGUNDO PASO (Notificación)
	// Esto debería disparar el Rollback
	notif.On("Publish", ctx, mock.Anything).Return(errors.New("notification service down"))

	// CONFIGURAMOS EL EXPECTATIVA DEL ROLLBACK
	// El helper debe llamar a Delete para borrar al usuario ID 123
	...
	// Verificamos que se haya intentado borrar al usuario (confirmando el rollback)
